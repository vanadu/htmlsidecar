...
var Witty = (function () {
  ...
  var UIController = (function() {
    ...
    function getTextInput( alias ) {
      console.log('getInputSelection running');
      let retVal;
      // !VA Replace the parent's id code with the input element's id code, get the input element and add its value to Appobj 
      retVal = document.querySelector(ccpUserInput[alias].replace('tfd', 'ipt')).value;
      return retVal;
    }
    ...
    // !VA Branch: OVERHAUL0824A
    function getCheckboxSelection( alias ) {
      console.log('getCheckboxSelection running');
      let retVal;
      // !VA Replace the parent's id code with the input element's id code, get the input element and add its value to Appobj 
      retVal = document.querySelector(ccpUserInput[alias].replace('chk', 'ipt')).value;
      return retVal;
    }
    
    // !VA Branch: OVERHAUL0824A
    // !VA CBController private
    // !VA Gets the code string for the selected radio element in a radio group container based on the alias/Appobj property of the radio group
    function getRadioSelection( alias ) {
      ...
      // !VA Loop through the child elements and get the checked element
      for (const el of iptElements) {
        if (el.checked) { selectedOption = el.id; }
      }
      // !VA Reduce the selected element id to its five-character option code
      selectedOption = selectedOption.substring( 8, 13 );
      // !VA If the passed-in alias 
      selectedOption = convertAlignCodes(selectedOption);
      return selectedOption;
    }

    // !VA There's a better way to do this, but this works for now
    function convertAlignCodes(selectedOption) {
      switch(true) {
      // !VA The element code is either vanon or hanon, so 'anon' gets them both
      case selectedOption.includes('anon') :
        selectedOption = 'none';
        break;
      case selectedOption === 'halft' :
        selectedOption = 'left';
        break;
      case selectedOption === 'hactr' :
        selectedOption = 'center';
        break;
      case selectedOption === 'hargt' :
        selectedOption = 'right';
        break;
      case selectedOption === 'vatop' :
        selectedOption = 'top';
        break;
      case selectedOption === 'vamid' :
        selectedOption = 'middle';
        break;
      case selectedOption === 'vabtm' :
    ...
    function getCheckboxSelection(alias) {
      let iptElements, elementChecked;
      // !VA iptElements is the collection of child INPUT elements of the element with the id 'value' corresponding to the alias 'key'
      iptElements = document.querySelector(ccpUserInput[alias]).getElementsByTagName('INPUT');
      for (const el of iptElements) {
        if (el.checked) { elementChecked = true; }
        el.checked ? elementChecked = true : elementChecked = false;
      }
    ...
    return {
      ...
      populateAppobj: function (Appobj, access) {
        console.log('populateAppobj running');
        ...
        function populateAppProperties(Appobj) {
          console.log('populateAppProperties running');
        ...
        function populateCcpProperties(Appobj) {
          ...
          for (const [key, value] of Object.entries(ccpUserInput)) {
            ...
            if (key.substring( 11 ) === 'Tfd') {
              Appobj[ key ] = getTextInput( key );
            }
            // !VA Write the Align/Valgn radio input element values to Appobj
            if (key.substring( 11 ).includes('Rdo')) {
              // !VA getRadioSelection gets the selected option for the align, valign, TD optns, IMG excld and IMG itype radio elements, which are then written to Appobj
              Appobj[ key ] = getRadioSelection(key);
            }
            if ( key.substring( 11 ).includes('Chk')) {
              // !VA Write the checkbox element values to Appboj
              Appobj[ key ] = getCheckboxSelection(key);
            }
          }
...
style="background-color:#556270;background-image:url(${Attributes.imgSrc.str});border:1px solid #1e3650;border-radius:4px;color:#ffffff;display:inline-block;font-family:sans-serif;font-size:13px;font-weight:bold;line-height:${tdHeight}px;text-align:center;text-decoration:none;width:${tdWidth}px;-webkit-text-size-adjust:none;mso-hide:all;">Show me the button!</a></div>`;
  ...
  var appController = (function(CBCtrl, UICtrl) {
    ...
    function resetTdOptions() {
      ...
      // !VA 082420
      // !VA This is where OLD Appobj properties were being written, so fixing them now
      Appobj.ccpImgClassTfd = Appobj.ccpTdaClassTfd = Appobj.ccpTdaHeigtTfd = Appobj.ccpTdaWidthTfd = Appobj.ccpTblClassTfd = Appobj.ccpTdaBgclrTfd = '';
      Appobj.ccpTblWidthTfd = Appobj.curImgW;
      // !VA Branch: review0720F (071720)
      // !VA This is where the default for iptCcpTableWrapperWidth is set
      Appobj.ccpTblWraprChk = Appobj.imgViewerW;
      // !VA Branch: review0720F (071720)
      ...
      UIController.writeAppobjToDOM( 
        [ 'ccpImgClassTfd', Appobj.ccpImgClassTfd ], 
        [ 'ccpTblClassTfd', Appobj.ccpTblClassTfd ],  
        [ 'ccpTbwClassTfd', Appobj.ccpTbwClassTfd ], 
        ['ccpTdaWidthTfd', Appobj.ccpTdaWidthTfd ], 
        [ 'ccpTdaHeigtTfd', Appobj.ccpTdaHeigtTfd ], 
        ['ccpTblWidthTfd', Appobj.ccpTblWidthTfd ], 
        ['ccpTbwWidthTfd', Appobj.ccpTbwWidthTfd ], 
        ['ccpTdaBgclrTfd', Appobj.ccpTdaBgclrTfd ] );
