...
var witty = (function () {
  ...
  var UIController = (function() {
    ...
    function doDimViewers(obj) {
      ...
      // !VA NEW Need to calculate the viewer and appContainer sizes first then write the modified obj to doDimViewers
      // calcViewerDimensions(obj);

    }

    // !VA NEW THis is initialized from handleFileSelect with a call to UICtrl.writeImgDimensions. It should be renamed, since it now resizes all the dynamic UI elements, including curImg
    // UIController private: calViewerDimensions
    ...
    return {
      ...
      // !VA NEW
      // !VA NEW Write the filename to the dimViewer.filename - only has to be done once per handleFileSelect session, so no need to keep passing it in Appdata.
      ...
      buildDimViewers: function(obj) {
        // !VA NEW This initializes the process of populating dimViewers with data from the user input handler functions in appController. appController passes updated current image dimensions and UIController gets the current container values, adjusts them accordingly, and updates the app display. 
        // !VA NEW Now that we have the current image dimensions, we can do the dimViewers
        calcViewerDimensions(obj);
      },
      getCurImgDimensions() {
        ...
        curImg.imgW = curImg.width;
        curImg.imgH = curImg.height;
        curImg.imgNW = curImg.naturalWidth;
        curImg.imgNH = curImg.naturalHeight;
...
`<table ${tableTag.classAtt + ' '}${tableTag.alignAtt + ' '}${tableTag.widthAtt + ' '}${tableTag.bgcolorAtt + ' '}border="0" cellpadding="0" cellspacing="0">
  ...
  var appController = (function(calcCtrl, UICtrl) {
    ...
    // !VA Deprecated in this version
    // var ccpBuildTag = UICtrl.getCcpBuildTagIDs();
    ...
    var setupEventListeners = function() {
      ...
      for (let i = 0; i < ioKeypresses.length; i++) {
        ...
        addEventHandler((ioKeypresses[i]),'input',UICtrl.showElementOnInput,false);
        // !VA TODO: More commented handlers here, deleted for tidiness - see backups
      ...
      // !VA TODO: More commented handlers here, deleted for now -- see backups.
    };
          ...
          function initializeDimViewers() { 
            ...
            setTimeout(() => {
              ...
              // !VA Hide the drop area.
                ...
                curImgDimensions = UICtrl.getCurImgDimensions();


                UICtrl.buildDimViewers(curImgDimensions);
    ...
    // appController: UPDATE APP DATA
    // !VA NEW Delete
    function updateAppData (prop, val) {
      val = parseInt(val); 
      // !VA !IMPORTANT! Referencing a property of an object in bracket notation!
      Appdata[prop] = val;
      UICtrl.refreshAppUI(Appdata);
      // return Appdata;
    }
    ...
    function handleUserAction(e) {
      ...
      if (event.type === 'click') {
        ...
        if (keypressed == 13) {
          ...
          isErr = validateInteger(this.value);
          if (isErr) {
    ...
    var initializeDevMode = function() {
      // !VA NEW Get the dynamicRegions and dimViewer element names
      var dynamicRegions = UICtrl.getDynamicRegionIDs();
      var dimViewers = UICtrl.getDimViewerIDs();
      ...
      window.addEventListener('load', function() {
        ...
        // !VA NEW Not sure why that placeholder isn't just set in the HTML
        ...
        // !VA NEW Just image element from the HTML since it is a DOM node and its properties are queryable directly from the HTML in dev mode, unlike the blob provided by the File Reader object.
        var curImg = document.querySelector(dynamicRegions.curImg);
        // !VA NEW Get the dimensions, same obj as returned by UICtrl.getCurImgDimensions
        var obj =  {};
        obj.imgW = curImg.width;
        obj.imgNW = curImg.naturalWidth;
        obj.imgH = curImg.height;
        obj.imgNH = curImg.naturalHeight;
        // !VA Hide the drop area.
        document.querySelector(staticRegions.dropArea).style.display = 'none';
        // !VA  Show the toolbar
        document.querySelector(staticRegions.toolsContainer).style.display = 'block';
        // !VA NEW Initialize the process of populating the dimViewers.
        UICtrl.buildDimViewers(obj);
