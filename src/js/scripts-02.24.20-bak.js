...
var Whitty = (function () {
  ...
  var CBController = (function() {
    ...
    function ccpMakeImgTag(id, indentLevel) {
      ...
      // !VA Build the img tag and pass the outerHTML to writeClipboard as str OR return the img element to the caller ccpMakeTdTag. Called from CBController.doClipboard.
      let indent;
      // !VA Get the attributes
      let Attributes = getAttributes();
      // !VA Get the indent character depending on indentLevel
      ...
      // !VA Get this through your head: We can't add indents here because the insertAdjacentHTML method requires a parent element. indentLevel is irrelevant for the lowermost child in the hierarchy.
    ...
    function ccpMakeTdTag(id, indentLevel ) {
      ...
      // !VA Get the Attributes to determine which options are set
      var Attributes = getAttributes();
      ...
      // !VA We start with the two attri  butes that are present in both td variants.
      ...
      // !VA Now we build the td when tdBgimage is NOT checked
      if (!Attributes.tdBgimage) {
        console.log('tdBgimage not checked');
        // !VA Add class and align
        // !VA class attribute
        ...
        var img = ccpMakeImgTag(childId, indentLevel);
        // !VA If Include wrapper table is unchecked in table options, append the child img to the parent td. 
        td.appendChild(img);

        // !VA indentLevel is null - the CCP button is the caller, so set the indentLevel to 1 and call ccpMakeImgTag
        // indentLevel = 1;
        ccpMakeImgTag( id, indentLevel);
      ...
      // !VA Insert the indents before the img and the line break after it. indentLevel is 1, which is implied when ccpMakeImgTag is called from here. indentLevels don't become relevant until ccpMakeTableTag, when there are multiple children to indent. 
      // img.insertAdjacentHTML('beforebegin', indent);
      // img.insertAdjacentHTML('afterend', '\n');
      ...
      switch (true) {
      case (indentLevel === 1):
        console.log('case1: indentLevel is: ' + indentLevel);
        // !VA This function is called from the CCP button
        // !VA Write the snippet to the clipboard 
        var str = td.outerHTML;
        break;
      case (!indentLevel === 1):
        console.log('case2: indentLevel is: ' + indentLevel);
        // !VA don't set an indent -- let the table handler do it. Remove the img from the element
        td.removeChild[0];
        break;
      default:
        console.log('default');
    ...
    function ccpMakeTableTag(id, indentLevel ) {  
      ...
      // !VA trying branch -- original code is in Notes
      // !VA get the indent char




      console.log('here indentLevel is: ' + indentLevel);
      switch (true) {
      case (indentLevel === 3):
        // !VA indentLevel is null - the CCP button is the caller, so set the indentLevel to 3, call ccpMakeImgTag and set the indents
        tr.insertAdjacentHTML('beforebegin', '\n' + indent);
        tr.insertAdjacentHTML('afterend', '\n');

        td = ccpMakeTdTag( id, 3 );
        tr.appendChild(td);
        td.insertAdjacentHTML('beforebegin', '\n' + indent + indent);
        td.insertAdjacentHTML('afterend', '\n' + indent );

        // img = ccpMakeImgTag( id, indentLevel);
        // td.appendChild(img);
    ...
    function ccpMakeWrapperTableTag(id, indentLevel) { 
      ...
      // !VA tdInnerDefault is the td that is output when the Background image checkbox is NOT checked.
      let tdInnerDefault = document.createElement('td');
      let tdInnerBgimage = document.createElement('td');
      let img;
      ...
      // !VA Build the inner default td, i.e. Bgimage not checked
      trInner.appendChild(tdInnerDefault);
      tdInnerDefault.insertAdjacentHTML('beforebegin', '\n          ');
      tdInnerDefault.insertAdjacentHTML('afterend', '\n        ');
      // !VA Add the inner td attributes
      if (Attributes.tdAlign) { tdInnerDefault.align = Attributes.tdAlign; }
      // !VA valign attribute
      if (Attributes.tdValign) { tdInnerDefault.vAlign = Attributes.tdValign; }
      // !VA class attribute
      if (Attributes.tdClass) { tdInnerDefault.className = Attributes.tdClass; }
      // !VA bgcolor attribute. Pass the input value, don't prepend hex # character for now
      if (Attributes.tdBgcolor) { tdInnerDefault.bgColor = Attributes.tdBgcolor; }
      ...
      tdInnerDefault.appendChild(img);
    ...
    function newMakeImgTag(id, indentLevel  ) {
      // !VA img is the terminating element in the node, so indentLevel is 0 and nothing more is required than outputting it to the clipboard
      let superClip, superClipNodeList, imgNode, str;
      superClip = makeSuperClip( id, 0 );
      // console.dir(superClip);
      superClipNodeList = superClip.querySelectorAll('*');
      imgNode = superClipNodeList[6];
      str = imgNode.outerHTML;
      writeClipboard( id,  str );
      return imgNode;
    }

    function newMakeTdTag(id, option, indentLevel ) {
      let superClip, superClipNodeList, tdInnerNode, str, Attributes;
      // !VA Output depends on if the Bgimage attribute is checked, so get the attributes
      Attributes = getAttributes();
      let checked = Attributes.tdBgimage;
      if (!checked) {
        console.log('not checked');

      }
      console.log('newMakeTdTag running');
      superClip = makeSuperClip( id, 0 );
      // console.dir(superClip);
      superClipNodeList = superClip.querySelectorAll('*');
      tdInnerNode = superClipNodeList[5];
      str = tdInnerNode.outerHTML;
      // writeClipboard( id,  str );
      return tdInnerNode;
    }
    ...
    return {
      ...
      doClipboard: function(evt) {
        ...
        case (id === document.querySelector(btnCcpMakeClips.btnCcpMakeImgTag).id):
          ...
          newMakeImgTag(id, 0);
          // console.dir(superClip);
          // var foo = superClip.querySelectorAll('*');
          // console.log('foo is:');
          // console.dir(foo);
          // var blib = 'blib' + foo[5].outerHTML;
          // console.log('blib is: ' + blib);
          // writeClipboard( id, blib );
