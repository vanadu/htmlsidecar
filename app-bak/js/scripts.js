...
// !VA The only place to create the ghost tables seems to be at the very end of buildOutputNodeList, after all the conditional routines for all the makeClip buttons have been closed. Those conditionals handle the tdoptions basic, excludeimg, posswitch (for DOM node processing) and imgswap, bgimage and vmlbutton (for MS Conditional processing). MS Conditional code for the ghost table is returned from getGhostTable as an array containing the opening and closing MS conditional code blocks. This code can't be inserted before/after the wrapper table node using insertAdjacentHTML because the beforebegin parameter only works if the selected node has a parent. Although there is a parent node, i.e. the container div, immediately prior to creating the output nodelist outputNL with querySelectorAll, any adjacentHTML inserted to the wrapper table using the beforebegin parameter of insertAdjacentHTML will be stripped out again as soon as querySelectorAll is run. I don't see any way to insert HTML before the wrapper table node short of adding yet another node which would probably throw the whole indent scheme off. So it appears the only way to add the MS conditional code for the ghost table is after outputNL is converted to the clipboardStr with the outerHTML method. At that point, the DOM is no more and we can just handle it as text, as the last mutation before the Clipboard object writes it out.   
